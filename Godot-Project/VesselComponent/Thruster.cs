...
public partial class Thruster : PlaceableComponent
{
    ...
    public override void Place(Vector3 position, Vector3 scale)
    {
	    ...
	    double lengthSquared = _length * _length;
    ...
    {
	    AddRoundedBox(new Vector3(_width - 0.1, _height - 0.1, _length - _nozzleLength), new Vector3(0, 0, -_nozzleLength / 2) + position, _engineCornerRadius);

	    AddBox(new Vector3(_width, 0.1, _nozzleLength), new Vector3(0, _height / 2, (_length - _nozzleLength) / 2 ) + position);
	    AddBox(new Vector3(_width, 0.1, _nozzleLength), new Vector3(0, -_height / 2, (_length - _nozzleLength) / 2 ) + position);
	    AddBox(new Vector3(0.1, _height, _nozzleLength), new Vector3(_width / 2, 0, (_length - _nozzleLength) / 2 ) + position);
	    AddBox(new Vector3(0.1, _height, _nozzleLength), new Vector3(-_width / 2, 0, (_length - _nozzleLength) / 2 ) + position);

	    var startPos = -_height / 2;
	    int finCount = (int)(_height / _finSpacing);

	    for (int i = 0; i < finCount; i++)
	    {
		    var pos = startPos + _finSpacing * i + _finSpacing;
		    AddBox(new Vector3(_width, _finWidth, _nozzleLength), new Vector3(0, pos, (_length - _nozzleLength) / 2 ) + position);
	    }
    ...
    private void AddRoundedBox(Vector3 scale, Vector3 position, double radius)
    {
	    ...
	    var cylinderPosAa = new Vector3(position.X + scale.X / 2 - radius, position.Y, position.Z + scale.Z / 2 - radius);
	    var cylinderPosAb = new Vector3(position.X + scale.X / 2 - radius, position.Y, position.Z - scale.Z / 2 + radius);
	    var cylinderPosAc = new Vector3(position.X - scale.X / 2 + radius, position.Y, position.Z - scale.Z / 2 + radius);
	    var cylinderPosAd = new Vector3(position.X - scale.X / 2 + radius, position.Y, position.Z + scale.Z / 2 - radius);
	...
	var cylinderPosBa = new Vector3(position.X, position.Y + scale.Y / 2 - radius, position.Z + scale.Z / 2 - radius);
	var cylinderPosBb = new Vector3(position.X, position.Y + scale.Y / 2 - radius, position.Z - scale.Z / 2 + radius);
	var cylinderPosBc = new Vector3(position.X, position.Y - scale.Y / 2 + radius, position.Z - scale.Z / 2 + radius);
	var cylinderPosBd = new Vector3(position.X, position.Y - scale.Y / 2 + radius, position.Z + scale.Z / 2 - radius);
	...
	var cylinderPosCa = new Vector3(position.X + scale.X / 2 - radius, position.Y + scale.Y / 2 - radius, position.Z);
	var cylinderPosCb = new Vector3(position.X + scale.X / 2 - radius, position.Y - scale.Y / 2 + radius, position.Z);
	var cylinderPosCc = new Vector3(position.X - scale.X / 2 + radius, position.Y - scale.Y / 2 + radius, position.Z);
	var cylinderPosCd = new Vector3(position.X - scale.X / 2 + radius, position.Y + scale.Y / 2 - radius, position.Z);
